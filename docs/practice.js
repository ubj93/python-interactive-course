...
function diagnosticBridgeLinks(id) {
  const bridges = (DATA.bash_bridges || []).filter(bridge => bridge.diagnostic_ids.includes(id));
  return bridges.length ? `<div class="bash-bridges"><p>Optional Bash-to-Python bridges: three cards at the start of an existing lesson, then continue or return here. These are related topics to choose, not a diagnosis of your mistake.</p>${bridges.map(bridge => `<p><a href="#/learn/${bridge.lesson}?card=${bridge.card}">${esc(bridge.title)}</a><small>Needs: ${esc(bridge.prerequisites)}.</small></p>`).join("")}</div>` : "";
}
