...
vm.runInNewContext(source.slice(start, end) + `
  assert.equal(recordCard('1.1', 0, true, true), 1);
  assert.equal(recordCard('1.1', 0, true, true), 0);
  P.cards = {}; saveProgress(); P = loadProgress();
  assert.equal(recordCard('1.1', 0, true, true), 0);
  assert.equal(P.xp, 1);
  P = blankProgress();
  recordCard('1.1', 0, true, false);
  P.cards = {}; saveProgress(); P = loadProgress();
  assert.equal(recordCard('1.1', 0, true, true), 0);
  assert.equal(P.xp, 0);
